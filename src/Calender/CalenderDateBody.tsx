...
import {
  EventRangeDisplayCalculator
} from "./service/EventRangeDisplayCalculator";
  ...
  return (
    <tbody className={'date-body'} ref={tbodyRef}>
      <tr>
        ...
        {
          props.calenderDates.map((d) => <td
              className={'event-cell'}
              key={d.date.toISOString()}
              style={{position: 'relative'}}
            >
              {(new EventRangeDisplayCalculator((events[d.date.getDate()] ?? []).map(d => {
                return {
                  start: d.startDate,
                  end: d.endDate
                }
      ...
      {props.dateStructs.map((dateStruct) => (
        <tr key={dateStruct.hour}>
          <td className={'hour-label-cell'}>
            ...
            <span
              className={'hour-label-text-dummy-for-space'}>{dateStruct.label}</span>
          </td>
